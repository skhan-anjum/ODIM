...
start_account_session()
{
	...
	nohup /bin/svc-account-session --registry=consul --registry_address=${registry_address} --server_address=account-session:45101 --framework=GRPC  --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/account_session.log 2>&1 &
	PID=$!
	sleep 3
